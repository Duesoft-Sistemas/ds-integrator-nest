...
export class JwtServiceInternal {
    ...
    async generateToken(type: TokenType, payload: Payload): Promise<{ token: string, expiresIn: Date }> {
        ...
        const token = await this.jwtService.signAsync(Object.assign({}, payload), { expiresIn, secret });
